...
import org.TheGame.input.ai.NoPossibleMovementException;
...
public class EscapingFromTypesAI extends ReactiveToTypesAI {
	...
	public EscapingFromTypesAI(Collection<String> typeNames, Collection<String> teamNames) {
	...
	protected List<Action> reactTo(Element element) {
		...
		try {
			Direction direction = this
					.determineNextDirectionForEscaping(element);
	...
	protected Direction determineNextDirectionForEscaping(Element element)
			throws NoPossibleMovementException {
		Collection<Direction> possibleDirections = this
				.validDirectionsForMoving();
		...
		Direction directionToGoToElement = creepsPosition
				.directionToGoTo(element.getPosition());
		List<Direction> bestPossibleDirections = directionToGoToElement
				.oppositeDirections();

		// remove the directions that make me move to an invalid or occupied
		// position
		Iterator<Direction> i = bestPossibleDirections.iterator();
		while (i.hasNext()) {
			Direction aDirection = i.next();
			Position nextPosition = creepsPosition
					.positionAfterMovingIn(aDirection);
			if (!creepsMap.validPosition(nextPosition)
					|| !creepsMap.occupable(nextPosition)) {
				i.remove();
			}
		}

		// Take the chance to remove the first best directions so as not to
