...
public class DiceRoller {
	...
	public static boolean rollDiceWithProbability(int probability) {
		if (probability < 0 || probability > 100) {
			// TODO ¿be lenient and prob<0 -> prob = 0 and prob>100 -> prob =
			// 100?
			throw new IllegalArgumentException(
					"Wrong probability value, should be 0 <= probability <= 100, but probability = "
							+ probability + " .");
		}
		int lower = GameObjects.getInstance().getRandomNumberGenerator()
				.getLowerRange();
		int upper = GameObjects.getInstance().getRandomNumberGenerator()
				.getUpperRange();
		int random = GameObjects.getInstance().getRandomNumberGenerator()
				.randomNumber();
