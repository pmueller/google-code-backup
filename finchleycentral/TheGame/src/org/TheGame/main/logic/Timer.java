...
package org.TheGame.main.logic;
...
public class Timer {
	...
	public void notifyMeIn(long time, Notifiable notifiable) {
		...
		this.getNotifiablesQueue()
				.add(time + timeElapsedSinceStart, notifiable);
	}

	public void timePassed(long timeElapsedSinceStart) {
		this.setTimeElapsedSinceStart(timeElapsedSinceStart);
		SortedQueue<Long, Notifiable> notifiablesQueue = this
				.getNotifiablesQueue();
		while (!notifiablesQueue.isEmpty()
				&& notifiablesQueue.topKey() <= timeElapsedSinceStart) {
	...
	protected void setNotifiablesQueue(
			SortedQueue<Long, Notifiable> notifiablesQueue) {
	 ...
	 * 
	 * protected Notifiable notifiable; protected long time;
	 * 
	 * public Waiter(Notifiable notifiable, long time){
	 * this.setNotifiable(notifiable); this.setTime(time); }
	 * 
	 * protected Notifiable getNotifiable() { return this.notifiable; }
	 * 
	 * protected void setNotifiable(Notifiable notifiable) { this.notifiable =
	 * notifiable; }
	 * 
