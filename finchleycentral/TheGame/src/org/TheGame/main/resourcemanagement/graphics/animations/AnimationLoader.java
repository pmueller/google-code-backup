...
import org.TheGame.visualization.graphics.animations.AnimationFrame;
...
public class AnimationLoader {
	...
	public Animation getAnimation(String name) {
		ArrayList<File> animationDirectories = this
				.getDirectoriesPathToAnimation(name);
		// the animationDirectory is the last of the list
		File animationDirectory = animationDirectories.get(animationDirectories
				.size() - 1);
		AnimationDescriptorLoader animationDescriptorLoader = new AnimationDescriptorLoader(
				animationDirectories, this.getAnimationsDirectoryPath());
		AnimationDescriptor animationDescriptor = animationDescriptorLoader
				.getAnimationDescriptor();
		return this.generateAnimation(animationDescriptor, animationDirectory);
	}

	/**
	 * @param animationName
	 *            of the animation to be loaded
	 * @return all the directories that must be traveled from the "animations"
	 *         directory to reach the proper directory of the animation with the
	 *         requested name
	 */
	...
	/**
	 * Get the directory named directoryName that is contained by
	 * currentDirectory
	 */
	...
	protected Animation generateAnimation(
			AnimationDescriptor animationDescriptor, File animationDirectory) {
		ArrayList<AnimationFrameDescriptor> animationFrameDescriptors = animationDescriptor
				.getAnimationFrameDescriptors();
		ArrayList<Image> frameImages = this.getFrameImages(
				animationFrameDescriptors, animationDirectory);
		ArrayList<AnimationFrame> animationFrames = new ArrayList<AnimationFrame>();
		for (int i = 0; i < animationFrameDescriptors.size(); i++) {
			AnimationFrameDescriptor animationFrameDescriptor = animationFrameDescriptors
					.get(i);
			Image image = frameImages.get(i);
			AnimationFrame animationFrame = new AnimationFrame(image,
					animationFrameDescriptor.getPercentage());
	 ...
	 * @return
	 */
	private ArrayList<Image> getFrameImages(
			ArrayList<AnimationFrameDescriptor> animationFrameDescriptors,
	...
	protected File getAnimationsDirectory() {
		...
		if (!directory.exists()) {
			throw new GameLoadingError("Animations path invalid, " + path
					+ " does not exist");
		}
