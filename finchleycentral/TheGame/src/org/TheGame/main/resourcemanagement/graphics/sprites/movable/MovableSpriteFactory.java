...
package org.TheGame.main.resourcemanagement.graphics.sprites.movable;
...
public class MovableSpriteFactory {
	...
	public MovableSprite getSimpleMovableSprite(String name) {
		MovableSpriteCloner movableSpriteCloner = this.simpleMovableSpriteCloners
				.get(name);
	...
	public MovableSprite getMovableSprite(String name) {
		MovableSpriteCloner movableSpriteCloner = this.movableSpriteCloners
				.get(name);
		if (movableSpriteCloner == null) {
	...
	protected MovableSprite loadMovableSprite(String name) {
		...
		for (Direction direction : Direction.allDirections()) {
			...
			Animation animation = GameFactories.getInstance()
					.getAnimationFactory().getAnimation(animationName);
			animations.put(direction, animation);
		}
		return new MovableSprite(animations);
	}
