...
public class CreepDescriptionFactory {
	...
	public CreepDescriptionFactory(File creepDescriptionFolder) {
		this.creepDescriptions = this.loadCreepDescriptions(
				creepDescriptionFolder, "");
	}

	/**
	 * @param baseName
	 *            of the CreepDescriptions inside the creepDescriptionDirectory
	 * @param creepDescriptionDirectory
	 * @return a HashMap of names/CreepDescription pairs
	 */
	protected HashMap<String, CreepDescription> loadCreepDescriptions(
			File creepDescriptionDirectory, String baseName) {
		...
		for (File file : creepDescriptionDirectory.listFiles()) {
				...
				if (file.isDirectory()) {
					creepDescriptions.putAll(this.loadCreepDescriptions(file,
							newBaseName));
				} else if (file.isFile()) {
					// if its a file and it ends with the proper suffix, load its
					// description
					if (file.getName().endsWith(
							"." + this.getCreepDescriptionFileSuffix())) {
						// remove the suffix from the name of the description
						int endIndex = newBaseName.length() - 1
								- this.getCreepDescriptionFileSuffix().length();
						newBaseName = newBaseName.substring(0, endIndex);
						creepDescriptions.put(newBaseName, this
								.loadCreepDescription(file, newBaseName));
					}
				} else {
					GameFactories.getInstance().getFactoriesLogger().logWarning(
							"Warning: While loading creep descriptions: file "
									+ file.getPath()
	...
	protected String getCreepDescriptionFileSuffix() {
		return Configuration.getInstance().getValue(
				"creepDescriptionFileExtension");
	}

	/**
	 * @param file
	 *            : text file from which to load the creep's description
	 * @return
	 */
