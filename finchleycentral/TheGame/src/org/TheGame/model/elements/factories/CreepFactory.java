...
public class CreepFactory extends MovableElementFactory {
	...
	public CreepFactory() {
		...
		CreepDescriptionFactory creepDescriptionFactory = this
				.getCreepDescriptionLoader();
		// System.out.println(creepDescriptionFactory.getCreepDescriptions());
		this.getCreepDescriptions().putAll(
				creepDescriptionFactory.getCreepDescriptions());
	}

	public Creep createCreep(String name, TileMap tileMap, Position position)
	...
	public Creep createCreep(String name) {
		CreepDescription creepDescription = this.getCreepDescriptions().get(
				name);
	...
	protected CreepDescriptionFactory getCreepDescriptionLoader() {
		if (this.creepDescriptionFactory == null) {
			...
			String creepDescriptionsDirectoryName = Configuration.getInstance()
					.getValue("creepDescriptionsDirectory");
			String creepDescriptionsDirectory = Configuration.getInstance()
					.getRootDirectory()
					+ "" + creepDescriptionsDirectoryName;
			File creepDescriptionsDirectoryFile = new File(
					creepDescriptionsDirectory);
			this.creepDescriptionFactory = new CreepDescriptionFactory(
					creepDescriptionsDirectoryFile);
		}
		return this.creepDescriptionFactory;
	}

	protected void setCreepDescriptionLoader(
			CreepDescriptionFactory creepDescriptionFactory) {
	...
	protected void setCreepDescriptions(
			HashMap<String, CreepDescription> creepDescriptions) {
	...
	public Creep createRandomCreep() {
		...
		CreepDescription[] descriptions = this.getCreepDescriptions().values()
				.toArray(new CreepDescription[descriptionsQuantity]);
		int[] probabilities = this
				.getUniformProbabilities(descriptionsQuantity);
		RandomChooser<CreepDescription> randomChooser = new RandomChooser<CreepDescription>(
				descriptions, probabilities);
		return randomChooser.getObject().createCreep();
	}

	public Creep createRandomCreep(TileMap tileMap, Position position)
			throws InvalidPositionException, OccupiedPositionException {
