...
/**
 * State of a movable element when it is not moving. In this state, can start to
 * move.
 * 
...
public class IdleMovementState extends MovementState {
	...
	public void move(Direction direction, int moveSpeed) {
		...
		if (!movableElement.facesDirection(direction)) { // not moving! just
															// changing
															// direction
			movableElement.changeDirection(direction);
			movableElement.setMovementState(new ChangingDirectionMovementState(
					movableElement, moveSpeed / 3));
		} else {
			Position currentPosition = movableElement.getPosition();
			Position newPosition = currentPosition
					.positionAfterMovingIn(direction);
			try {
				movableElement.getCurrentMap().moveElement(currentPosition,
						newPosition);
				...
				if (direction.getHorizontalDirection() != 0
						&& direction.getVerticalDirection() != 0) { // 
					speed = movableElement.getDiagonalSpeed(moveSpeed);
				} else {
					speed = moveSpeed;
				}
				movableElement.setMovementState(new MovingMovementState(
						movableElement, speed));
				movableElement
						.getElementType()
						.getCharacteristics()
						.usedAttributeWithIntensityAndProbability("Speed", 1, 3);
				movableElement.getElementType().getCharacteristics()
						.usedAttributeWithIntensityAndProbability("Endurance",
								1, 3);
