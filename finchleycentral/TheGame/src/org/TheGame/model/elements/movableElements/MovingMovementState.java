...
public class MovingMovementState extends MovementState {
	...
	public MovingMovementState(MovableElement movableElement,
			long movingBlockedTime) {
	...
	public void update(long timeElapsed) {
		/*
		 * it's 10 and not 0 to make it 0 on average, because you can't depend
		 * on getting here every ms, so most of the time you caught it on -5,
		 * -10, -20 and so on. the value 10 was obtained empirically; of course,
		 * it would depend on the machine the game is played.
		 */
		long newBlockedTime = this.getMovingBlockedTime() - timeElapsed;
		if (newBlockedTime <= 10) {
			this.getMovableElement().setMovementState(
					new IdleMovementState(this.getMovableElement()));
