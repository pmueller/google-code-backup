...
import org.TheGame.model.elements.movableElements.statistics.StatisticObserver;
import org.TheGame.util.TimePartitioner;
...
public class AttributeState implements StatisticObserver {
	 ...
	 *            : One unit per every restorationRate milliseconds
	 */

	public AttributeState(String name, int startingValue,
			Statistic maximumValue, Statistic restorationRate) {
		this.setName(name);
		this.setCurrentValue(startingValue);
		this.setMaximumValue(maximumValue);
		this.setRestorationRate(restorationRate);
		this.setTimePartitioner(new TimePartitioner((int) restorationRate
				.getValue()));
	...
	public void statisticChanged(Statistic statistic) {
		if (statistic.equals(this.getRestorationRate())) {
			this.getTimePartitioner().changePartitionDuration(
