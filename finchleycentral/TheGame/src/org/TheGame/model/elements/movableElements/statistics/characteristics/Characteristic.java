...
/**
 * A Characteristic of a character (player, creep or NPC) that gets
 * improved/worsened by the actions of the player. Can be an Attribute, like
 * Strength, Intelligence, or a Skill like Dodge or Medicine
 * 
...
public abstract class Characteristic extends Statistic {
	...
	public Characteristic(String name, int experiencePointsModifierPerUse,
			List<Integer> experiencePointsRequiredPerLevel) {
		super(name);
		this.setExperienceKeeper(new ExperienceKeeper(
	...
	public int getExperiencePointsRequiredForCurrentLevel() {
		return this.getExperienceKeeper()
	...
	public void resetCurrentLevelTo(int tentativeLevel, int experiencePoints) {
		this.experienceKeeper.resetCurrentLevelTo(tentativeLevel,
				experiencePoints);
	}
	...
	public String toString() {
		...
		String string = this.getName() + ": " + level + " (" + currentExp + "/"
				+ requiredExp + ")";
		return string;
	}
}
