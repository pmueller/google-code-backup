...
package org.TheGame.model.elements.movableElements.statistics.characteristics;

import java.util.ArrayList;
import java.util.List;

/**
 * A class that given a list of experience requirements per level, manages the
 * adding of experience, as well as the possible increase in level
 * 
...
public class ExperienceKeeper {
	...
	/**
	 * The max level is implicit as the size of the
	 * experiencePointsRequiredPerLevel list. When the max level is reached,
	 * further attempts to increase the experience points will result in nothing
	 * happening
	 * 
	...
	public ExperienceKeeper(List<Integer> experiencePointsRequiredPerLevel) {
		...
		this.getExperiencePointsRequiredPerLevel().addAll(
	 ...
	 * @param amount
	 *            of xp to add
	 * @returns true if the adding of experience points caused an increase to
	 *          the next level
	 */
	public boolean addExperiencePoints(int amount) {
		...
		if (!this.reachedMaxLevel()) {
			int newExperiencePoints = this.getExperiencePointsOfCurrentLevel()
					+ amount;
			if (newExperiencePoints >= this
					.experiencePointsRequiredForCurrentLevel()) {
	...
	public boolean reachedMaxLevel() {
		return this.getCurrentLevel() == (this.maxLevel()-1);
	}
	...
	/**
	 * @return the amount of experience points that are needed to pass to the
	 *         next level
	 */
	public int experiencePointsRequiredForCurrentLevel() {

		return this.getExperiencePointsRequiredPerLevel().get(
	...
	protected void setExperiencePointsRequiredPerLevel(
