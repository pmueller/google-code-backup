...
package org.TheGame.model.elements.movableElements.statistics.characteristics.skills;

import java.util.ArrayList;
import java.util.List;

import org.TheGame.exceptions.GameError;
import org.TheGame.model.elements.movableElements.statistics.characteristics.Characteristic;
import org.TheGame.model.elements.movableElements.statistics.characteristics.attributes.Attribute;

/**
 * A specific Characteristic of an Element, it represents it's ability in a
...
public class Skill extends Characteristic {
	...
	public Skill(String name, int experiencePointsModifierPerUse,
			List<Integer> experiencePointsRequiredPerLevel) {
		super(name, experiencePointsModifierPerUse,
				experiencePointsRequiredPerLevel);
	...
	public Skill(String name, int experiencePointsModifierPerUse,
			List<Integer> experiencePointsRequiredPerLevel,
			List<Attribute> dependingAttributes,
			List<Integer> dependingAttributesModifier) {
		this(name, experiencePointsModifierPerUse,
				experiencePointsRequiredPerLevel);
		if (dependingAttributes.size() != dependingAttributesModifier.size()) {
			String message = "The number of attributes must be the same as the number of attribute modifiers ("
					+ dependingAttributes.size()
					+ " vs "
					+ dependingAttributesModifier.size() + ")";
			throw new GameError(message);
		}
		this.getDependingAttributes().addAll(dependingAttributes);
		this.getDependingAttributesModifier().addAll(
				dependingAttributesModifier);
	}
	...
	private void triggerUseOfDependingAttributes(int intensity) {
		for (int i = 0; i < this.getDependingAttributes().size(); i++) {
			...
			attribute.usedWith(intensity
					* this.getDependingAttributesModifier().get(i));
		}
	}
	...
	private void triggerUseOfDependingSkills(int intensity) {
		for (int i = 0; i < this.getDependingSkills().size(); i++) {
			...
			skill.usedWithByDependent(intensity
					* this.getDependingSkillsModifier().get(i));
		}
	}
	...
	protected void setDependingSkillsModifier(
			List<Integer> dependingSkillsModifier) {
	...
	protected void setDependingAttributesModifier(
			List<Integer> dependingAttributesModifier) {
