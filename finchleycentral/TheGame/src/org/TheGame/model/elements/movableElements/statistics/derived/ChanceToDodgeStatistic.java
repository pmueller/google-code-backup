...
public class ChanceToDodgeStatistic extends DerivedStatistic {
	...
	public static ChanceToDodgeStatistic newChanceToDodge(float baseValue,
			Attribute speed, Attribute dexterity, Attribute concentration) {
		...
		return new ChanceToDodgeStatistic("ChanceToDodge", baseValue,
				statistics);
	}

	public ChanceToDodgeStatistic(String name, float baseValue,
	...
	public float calculate() {
		float value = this.getBaseValue()
				+ (this.getValueOfDependingStatistic("Concentration") / 3
						+ this.getValueOfDependingStatistic("Speed") / 5 + this
						.getValueOfDependingStatistic("Dexterity") / 5);
