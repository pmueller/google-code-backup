 ...
 */
public class ChanceToHitStatistic extends DerivedStatistic {

	public static ChanceToHitStatistic newChanceToHit(float baseValue,
			Attribute speed, Attribute dexterity, Skill weapons) {
		ArrayList<Statistic> statistics = new ArrayList<Statistic>();
		;
		statistics.add(speed);
		statistics.add(dexterity);
		statistics.add(weapons);
		return new ChanceToHitStatistic("ChanceToHit", baseValue, statistics);
	}

	public ChanceToHitStatistic(String name, float baseValue,
	...
	public float calculate() {
		float value = this.getBaseValue()
				+ (this.getValueOfDependingStatistic("Weapons") / 3
						+ this.getValueOfDependingStatistic("Speed") / 10 + this
						.getValueOfDependingStatistic("Dexterity") / 3);
