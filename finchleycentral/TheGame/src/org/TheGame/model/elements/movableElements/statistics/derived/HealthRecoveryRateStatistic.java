 ...
 */
public class HealthRecoveryRateStatistic extends DerivedStatistic {

	public static HealthRecoveryRateStatistic newHealthRecoveryRate(
			float baseValue, Attribute constitution) {
		ArrayList<Statistic> statistics = new ArrayList<Statistic>();
		;
		statistics.add(constitution);
		return new HealthRecoveryRateStatistic("HealthRecoveryRate", baseValue,
				statistics);
	}

	public HealthRecoveryRateStatistic(String name, float baseValue,
	...
	public float calculate() {
		float value = this.getBaseValue()
				- this.getValueOfDependingStatistic("Constitution") / 5;
