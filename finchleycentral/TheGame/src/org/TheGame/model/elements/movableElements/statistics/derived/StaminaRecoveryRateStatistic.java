...
public class StaminaRecoveryRateStatistic extends DerivedStatistic {
	...
	public static StaminaRecoveryRateStatistic newStaminaRecoveryRate(
			float baseValue, Attribute endurance) {
		ArrayList<Statistic> statistics = new ArrayList<Statistic>();
		;
		statistics.add(endurance);
		return new StaminaRecoveryRateStatistic("StaminaRecoveryRate",
				baseValue, statistics);
	}

	public StaminaRecoveryRateStatistic(String name, float baseValue,
	...
	public float calculate() {
		float value = this.getBaseValue()
				- (this.getValueOfDependingStatistic("Endurance") / 2);
