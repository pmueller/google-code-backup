...
import org.TheGame.model.entities.NullElement;
import org.TheGame.model.entities.NullFloorCell;
import org.TheGame.model.map.maputils.Area;
import org.TheGame.model.map.maputils.Position;

/**
 * A TileMap where the game objects are held and move; it has a certain size and
 * two layers, one for floorTiles (ie grass, dirt, water), one for elements
 * (trees, players, creeps, rocks)
 * 
...
public class TileMap extends AbstractTileMap {
	...
	/**
	 * @return true if the floor layer at this position is walkable and there
	 *         are not any elements in the position.
	 */
	public boolean occupable(Position position) {
		return (!this.existsElementAt(position) && this.getFloor(position)
				.isWalkable());
	}

	public void moveElement(Position currentPosition, Position newPosition)
	...
	public void addElementAt(Element element, Position position)
		...
		if (!this.validPosition(position)) {
			...
			throw new OccupiedPositionException("Block at position " + position
					+ " is occupied or unwalkable");
		}
	...
	public Area area() {
		return new Area(Position.newPosition(0, 0), Position.newPosition(this
				.getWidth() - 1, this.getHeight() - 1));
	}

	/**
	 * @param position
	 *            that is to be tested as valid
	...
	public boolean validPosition(Position position) {
		...
		/*
		 * int x = position.getX(); int y = position.getY(); return (x >= 0 && y
		 * >= 0 && x < this.getWidth() && y < this.getHeight());
		 */
	}
}
