 ...
 */
package org.TheGame.model.map.maputils;

import java.util.ArrayList;
import java.util.Collection;
import java.util.Iterator;

import org.TheGame.util.NumberRange;

/**
 * An area of a map, think of it as a rectangle with from as the top-left
 * position and to as the bottom-right position.
 * 
...
public class Area implements Iterable<Position> {
	 ...
	 */
	public static Area newAreaAround(Position center, int radius)
			throws IllegalArgumentException {
		if (radius < 0) {
			throw new IllegalArgumentException("Radius should be >= 0");
		}
		Position from = Position.newPosition(center.getX() - radius, center
				.getY()
				- radius);
		Position to = Position.newPosition(center.getX() + radius, center
				.getY()
				+ radius);
	...
	protected void updateRanges() {
		this.setVerticalRange(new NumberRange(this.getFrom().getY(), this
				.getTo().getY()));
		this.setHorizontalRange(new NumberRange(this.getFrom().getX(), this
				.getTo().getX()));
	}
	 ...
	 * @return if position is contained in the Area (in the rectangle the Area
	 *         forms)
	 */
	public boolean contains(Position position) {
		boolean verticallyIncluded = this.verticalRange.contains(position
				.getY());
		boolean horizontallyIncluded = this.horizontalRange.contains(position
				.getX());
		return verticallyIncluded && horizontallyIncluded;
	}

	public boolean properlyContains(Position position) {
		boolean properlyVerticallyIncluded = this.verticalRange
				.properlyContains(position.getY());
		boolean properlyHorizontallyIncluded = this.horizontalRange
				.properlyContains(position.getX());
	...
	/**
	 * @return an iterator that iterates over every position this area contains
	 *         in a left to right, up to down order.
	 */
	...
	public String toString() {
		return "Area from: " + this.getFrom().toString() + " to: "
				+ this.getTo().toString();
	}
