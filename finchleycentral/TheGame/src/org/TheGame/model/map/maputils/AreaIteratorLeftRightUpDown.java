...
/**
 * Iterates over every position of the area contains in a left to right, up to
 * down order
 * 
...
public class AreaIteratorLeftRightUpDown implements Iterator<Position> {
	...
	public Position next() {
		...
		if (this.getArea().horizontalRange().getTo() == previousPosition.getX()) {
			newPosition = Position.newPosition(this.getArea().getFrom().getX(),
					previousPosition.getY() + 1);
		} else { // go to next column of the row
			newPosition = Position.newPosition(previousPosition.getX() + 1,
