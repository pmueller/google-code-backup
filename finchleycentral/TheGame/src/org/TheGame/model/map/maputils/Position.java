...
public class Position {
	...
	public boolean equals(Position position) {
		return (this.getX() == position.getX() && this.getY() == position
				.getY());
	}

	/**
	 * Returns a new Position considering a move in a Direction from current
	 * Position
	 */
	...
	/**
	 * Returns true if positions are inmediate neigbours (can be reached in one
	 * movement). Note: returns false if positions are equal
	 */
	...
	public String toString() {
		return ("x=" + new Integer(this.getX()).toString() + " y=" + new Integer(
				this.getY()).toString());
	}
	...
	public Direction directionToGoTo(Position position) {
		if (position.equals(this)) {
			throw new InvalidPositionParameterException(
					"There's no possible direction to go from a position to itself");
		}
