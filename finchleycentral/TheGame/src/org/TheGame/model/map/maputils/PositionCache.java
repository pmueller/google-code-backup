...
/**
 * A cache of positions, so as not to waste too much memory because positions
 * are ubiquitous. has a absoluteMaximum that indicates how much in each four
 * directions it caches positions, regardless of sign. So if size = 10, it'll
 * cache positions from -10,-10 to 10,10, passing through -10,10 , 0,0 and
 * 10,-10 This class is important for performance so, on purpose, it is written
 * with as few method calls as possible.
 * 
...
public class PositionCache {
	...
	protected PositionCache(int absoluteMaximum) {
		if (absoluteMaximum <= 0) {
			throw new GameError(
					"Position cache - absoluteMaximum cannot be negative or zero");
		}
	...
	protected int arraySize() {
		return (this.absoluteMaximum * 2) + 1;// add 1 to account for 0,0 / 0,x
												// / x,0 positions
	}

	public boolean inRange(int x, int y) {
		boolean inVerticalRange = (x <= this.positiveBorder)
				&& (x >= this.negativeBorder);
		boolean inHorizontalRange = (y <= this.positiveBorder)
				&& (y >= this.negativeBorder);
		return inVerticalRange && inHorizontalRange;
	}

	protected void setPositionAt(int x, int y, Position position) {
		if (!this.inRange(x, y)) {
			throw new InvalidPositionParameterException("position " + x + ","
					+ y + " is not in range");
		}
