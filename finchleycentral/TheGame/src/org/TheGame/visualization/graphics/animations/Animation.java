...
package org.TheGame.visualization.graphics.animations;
...
/**
 * Manages a series of images (frames) and the amount of time to display each
 * frame. Each AnimationFrame knows the percentage of time it will be displayed.
 * The totalDuration specifies how long the entire animation lasts, in
 * milliseconds. The amount of time (in milliseconds) each frame must be
 * displayed is calculated as (percentage * totalDuration ) / 100
 * 
...
public class Animation {
	 ...
	 */
	public static Animation newAnimation(AnimationFrame animationFrame,
			AnimationFrame animationFrame2, long duration) {
		ArrayList<AnimationFrame> frames = new ArrayList<AnimationFrame>();
		frames.add(animationFrame);
	...
	public static Animation newAnimation(
			ArrayList<AnimationFrame> animationFrames, int duration) {
	 ...
	 */
	public Animation(ArrayList<AnimationFrame> frames, long totalDuration,
			boolean loops) {
		if (!framesPercentagesSum100(frames)) {
			throw new IllegalArgumentException(
					"Frames percentages sum should be equal to 100");
		}
		this.frames = frames;
	...
	public Animation clone() {
		return new Animation(new ArrayList<AnimationFrame>(frames),
				totalDuration, this.shouldLoop());
	}
	...
	protected void checkIfFrameTimeEnded() {
		...
		// and if the current frame's time duration has passed, advance to the
		// next frame
		if (!this.currentFrameIsLastFrame() || this.shouldLoop()) {
			long currentFrameDuration = this
					.getFrameDuration(currentFrameIndex);
			if (frameTime >= currentFrameDuration) {
				frameTime = frameTime - currentFrameDuration;
				currentFrameIndex = (currentFrameIndex + 1)
						% this.getNumberOfFrames();
				// if the time elapsed was greater than the frame to which we
				// advanced
	...
	public void setTotalDuration(long totalDuration) {
		...
		// If a part of the remainder cannot be evenly distributed, do it
		// sort-of-randomly.
		...
		if (remainder > 0) {
			...
			int unevenRemainder = remainder
					- (evenlyDistributedTime * numberOfFrames);
			for (int i = 0; i < numberOfFrames; i++) {
				framesDuration.set(i, framesDuration.get(i)
						+ evenlyDistributedTime);
			}
