...
public class GraphicMovingMovementState extends GraphicMovementState {
	...
	protected GraphicMovingMovementState(
			MovableElementGraphic movableElementGraphic,
			MovableSprite movableSprite, Direction direction, int speed) {
		...
		// set position opposite to where we are moving (so it'll look as if we
	...
	public void changeDirection(Direction direction) {
		throw new GraphicMovementLogicException(
				"Cannot change direction when moving!");
	}
	...
	public void update(long timeElapsed) {
		...
		time += timeElapsed;
		if (((newX * this.getDirection().getHorizontalDirection()) >= 0)
				&& ((newY * this.getDirection().getVerticalDirection()) >= 0)) {
			this.setY(0);
			this.setX(0);
		} else {
			this.setY(newY);
